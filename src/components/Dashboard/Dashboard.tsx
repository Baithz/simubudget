...
export function Dashboard() {
  ...
  const accountingSignal = useAccountingStore(
    (s) =>
      `${s.activeMonth}:${s.incomes.length}:${s.expenses.length}:${s.monthlyExpenses.length}:${s.closedMonths.length}`
  );
      ...
      <div className="flex h-full flex-col items-center justify-center gap-6 px-8 text-center">
        ...
        <div>
          ...
          <p className="mt-2 max-w-sm text-sm font-medium" style={{ color: "var(--text-muted)" }}>
            Configurez votre profil financier pour obtenir votre analyse personnalisée et votre
  ...
  const rdvColor =
    rdv >= 600 ? "var(--fin-green)" : rdv >= 200 ? "var(--fin-amber)" : "var(--fin-red)";

  const holderName =
    `${profile.holder?.firstName ?? ""} ${profile.holder?.lastName ?? ""}`.trim() ||
    profile.city ||
    "Profil";
  ...
  return (
    ...
    >
      ...
      <motion.div variants={stagger.item} className="flex items-start justify-between">
        ...
        <button onClick={() => navigate("/onboarding")} className="btn-ghost text-xs">
        ...
        <div className="premium-card p-6 flex flex-col justify-between md:col-span-2">
          ...
          <div className="mt-5 space-y-0 divide-y" style={{ borderColor: "var(--border)" }}>
            ...
            {breakdown.totalAids > 0 && (
              <BudgetRow
                label="Aides estimées (APL…)"
                value={breakdown.totalAids}
                sign="positive"
              />
            )}
          ...
          >
            ...
            >
              <svg viewBox="0 0 14 14" fill="currentColor" className="h-3.5 w-3.5">
                <path
                  fillRule="evenodd"
                  d="M7 1a6 6 0 1 0 0 12A6 6 0 0 0 7 1Zm-.75 3.5a.75.75 0 0 1 1.5 0v.75a.75.75 0 0 1-1.5 0V4.5ZM7 7.5a.75.75 0 0 0-.75.75v2a.75.75 0 0 0 1.5 0v-2A.75.75 0 0 0 7 7.5Z"
                  clipRule="evenodd"
                />
              </svg>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-0.5 text-xs font-medium" style={{ color: "var(--text-secondary)" }}>
                ...
                <button
                  ...
                  onClick={() =>
                    void openExternal(
                      "https://www.caf.fr/allocataires/droits-et-prestations/s-informer-sur-les-aides/simulateur-caf"
                    )
                  }
...
function ScoreLine({
  ...
  const barColor =
    pct >= 70 ? "var(--fin-green)" : pct >= 40 ? "var(--fin-amber)" : "var(--fin-red)";
